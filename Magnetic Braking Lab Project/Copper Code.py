...
labels(ax0,'Angular Velocity / $rad s^{-1}$','Time / s','Copper')

fig1 = plt.figure(figsize = (5,5))
ax1 = fig1.add_subplot(1,1,1)
labels(ax1,'Angular Velocity / $rad s^{-1}$','Time / s','Copper')

fig2 = plt.figure(figsize = (5,5))
ax2 = fig2.add_subplot(1,1,1)
labels(ax2,'Angular Velocity / $rad s^{-1}$','Time / s','Copper')
...
vals = pd.read_csv('Coppermag.csv', header = 0, names = ('t1','v1','t2','v2','t3','v3','t4','v4','t5','v5','t6','v6'))
...
nomag_newvals = nomag_vals[(nomag_vals['voltage']>(nomag_vals['voltage'].min()+10))&(nomag_vals['voltage']<(nomag_vals['voltage'].max()-20))]
...
for i in range (1,7):
    ...
    newvals = vals[(vals[f'v{i}']>(vals[f'v{i}'].min()+10))&(vals[f'v{i}']<(vals[f'v{i}'].max()-15))]
